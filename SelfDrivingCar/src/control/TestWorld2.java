...
public class TestWorld2 extends World {
	...
	private static int panSpeed = 20;
	...
	private void respondToKeyEvent() {
		...
		if ("c".equals(key) || "C".equals(key)) {
			if (isFocused && !cars.isEmpty()) {
				Car current = cars.get(currentCenter);
				if (getObjects(Car.class).contains(current)) {
					isFocused = false;
					currentCenter = 0;
					for(Ray ray : current.getRays()){
						removeObject(ray);
					}
					removeObject(current.getFront());
					removeObject(current.getRear());
